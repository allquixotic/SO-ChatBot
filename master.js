...
/** Test if the blob constructor works, and if not, fall back to BlobBuilder. Supports older browser engines from the early days of HTML5. */
var blob = null;
if(doesBlobConstructorWork()) {
	blob = new Blob( [worker_code], { type : 'application/javascript' } );
}
else {
	var b_b = new BlobBuilder();
	b_b.append([worker_code]);
	blob = b_b.getBlob('application/javascript');
}
